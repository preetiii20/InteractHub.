...
import { LayoutDashboard, Users, MessageSquare, Video, BarChart3, FileText, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
...
const menu = [
    ...
    { to: '/dashboard/admin/comms', label: 'Communications', icon: MessageSquare },
    { to: '/dashboard/admin/live', label: 'Live Communication', icon: Video },
    { to: '/dashboard/admin/monitoring', label: 'Live Monitoring', icon: BarChart3 },
...
const AdminLayout = () => {
    ...
    useEffect(() => {
        const shouldCollapse = 
            location.pathname.startsWith('/dashboard/admin/live') ||
            location.pathname.startsWith('/dashboard/admin/comms');
        setSidebarCollapsed(shouldCollapse);
    }, [location.pathname]);

    // Setup global communication notifications
    const userEmail = authHelpers.getUserEmail() || authHelpers.getUserName();
    useCommunicationNotifications(userEmail, me);
    ...
    return (
        <div className="flex flex-col h-screen w-full overflow-hidden bg-white">
            {/* Full Width Header */}
            <header className="h-20 flex-shrink-0 bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-700 border-b-4 border-blue-900 px-8 flex items-center justify-between shadow-2xl">
                <div className="flex items-center gap-4 h-full">
                    ...
                    <div className="text-white text-base font-bold opacity-100 tracking-widest drop-shadow-md">ADMIN PORTAL</div>
                </div>
                <div className="flex items-center gap-6 h-full">
                    ...
                    <div className="flex items-center gap-3 bg-blue-500 bg-opacity-40 px-5 py-2.5 rounded-full hover:bg-opacity-50 transition-all duration-200 border border-blue-300 border-opacity-50">
                        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center text-white font-bold text-lg shadow-lg border-2 border-white">
                            {me.charAt(0).toUpperCase()}
                        </div>
                        <div className="text-white text-base font-bold drop-shadow-lg">{me}</div>
                    </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                        ...
                        {menu.map(item => (
                            <NavLink key={item.to} to={item.to}
                                className={isActive(item.to, item.exact) 
                                    ? `flex items-center px-5 py-4 rounded-2xl bg-indigo-600 text-white font-medium transition-all duration-200 group relative border-l-4 border-white ${sidebarCollapsed ? 'flex-col justify-center' : 'gap-4'}`
                ...
                {/* Main Content */}
                <main className={`flex-1 flex flex-col ${location.pathname.startsWith('/dashboard/admin/live') ? 'overflow-hidden' : 'overflow-y-auto'} bg-gradient-to-br from-indigo-50 via-blue-50 to-purple-50`}>
                    <div className={`flex-1 ${location.pathname.startsWith('/dashboard/admin/live') ? 'overflow-hidden p-0' : 'overflow-y-auto p-8'}`}>
