...
import { LayoutDashboard, User, Users, MessageSquare, Briefcase, Megaphone, LogOut } from 'lucide-react';
...
const ManagerLayout = () => {
  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-hidden bg-slate-100">
      {/* Full Width Header */}
      <header className="h-20 flex-shrink-0 bg-gradient-to-r from-green-600 via-green-500 to-emerald-600 border-b border-green-700 px-8 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-4 h-full">
          <div className="text-3xl font-bold text-white">Interacthub</div>
          <div className="h-8 w-0.5 bg-white opacity-40"></div>
          <div className="text-white text-sm font-semibold opacity-90 tracking-wide">MANAGER PORTAL</div>
        </div>
        <div className="flex items-center gap-6 h-full">
          <div className="flex items-center gap-3 bg-white bg-opacity-15 px-4 py-2 rounded-full hover:bg-opacity-25 transition">
            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-emerald-600 font-bold text-sm">
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            {menu.map(item => (
              <NavLink key={item.to} to={item.to}
                className={isActive(item.to, item.exact) 
                  ? 'flex items-center justify-between px-5 py-4 rounded-2xl bg-emerald-600 text-white font-medium transition-all duration-200 group'
        ...
        {/* Main Content */}
        <main className="flex-1 flex flex-col overflow-hidden bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50">
