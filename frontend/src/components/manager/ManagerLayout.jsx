...
const menu = [
  { to: '/dashboard/manager', label: 'Dashboard', icon: '📊', exact: true },
  { to: '/dashboard/manager/employees', label: 'Employees', icon: '👤' },
  { to: '/dashboard/manager/groups', label: 'Project Groups', icon: '👥' },
  { to: '/dashboard/manager/communication', label: 'Live Communication', icon: '🗣️' },
  { to: '/dashboard/manager/projects', label: 'Projects', icon: '🧩' },
  { to: '/dashboard/manager/comms', label: 'Global Comms', icon: '📣' }
];
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-slate-100">
      <aside className="w-64 flex flex-col h-full flex-shrink-0 bg-slate-900 text-slate-100">
        <div className="px-6 py-5 text-2xl font-bold border-b border-slate-800 flex-shrink-0">Manager Portal</div>
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {menu.map(item => (
            <NavLink key={item.to} to={item.to}
              className={isActive(item.to, item.exact) ? 'flex items-center justify-between px-3 py-2 rounded-md bg-emerald-600 text-white'
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="h-16 flex-shrink-0 bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-600 border-b border-emerald-700 px-8 flex items-center justify-between shadow-lg">
          <div className="flex items-center gap-4 h-full">
            <div className="text-2xl font-bold text-white">🚀 Interacthub</div>
            <div className="h-6 w-0.5 bg-white opacity-40"></div>
            <div className="text-white text-xs font-semibold opacity-90 tracking-wide">MANAGER PORTAL</div>
          </div>
          <div className="flex items-center gap-6 h-full">
            <div className="flex items-center gap-3 bg-white bg-opacity-15 px-4 py-2 rounded-full hover:bg-opacity-25 transition">
              <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-emerald-600 font-bold text-sm">
